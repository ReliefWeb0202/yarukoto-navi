  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400 || response.status === 403) {
        throw new Error(`APIキーが無効、または権限がありません (${response.status}: ${errorMsg})`);
      } else if (response.status === 429) {
        throw new Error('APIの利用上限に達しました。時間を置いて再試行してください。');
      } else {
        throw new Error(`通信エラー (${response.status}: ${errorMsg})`);
      }
    }
    ...
    // JSON配列形式の部分だけを正規表現で抽出（パースエラー防止）
...
function renderTodoList() {
  ...
  if (todos.length === 0) {
    todoListEl.innerHTML = `
      <li class="todo-item" style="justify-content: center; color: var(--text-muted);">
